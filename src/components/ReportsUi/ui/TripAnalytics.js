...
const TripSection=["SR No.","Asset Name","Start Time","End Time","Start Location","End Location","Trip Duration","Engine ON time","In Motion Duration","Idling Duration","Stops Duration","Trip Mileage"]
    const dummyData = [
        {
            sr: 1,
            asset: "Toyota",
    ...
    return (
      <div
        className={`text-black mx-10 mt-4`}
      >
        <div className={`flex flex-row gap-3 p-2 border-2 border-[#e2e8f0] rounded-md min-h-55`}>
          <table className={`h-full`}>
            ...
            {trips.map((value, index) => {
                return (
                    ...
                    <tr key={index} className={`text-gray-700 border-y-2 border-[#e2e8f0]`}>
